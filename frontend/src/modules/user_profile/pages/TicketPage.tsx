...
export default function TicketListView() {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      try {
        const res = await api.get("/tickets"); // baseURL + /api/v1 viene de axios.ts
  ...
  const filteredTickets = tickets.filter((ticket) => {
    ...
    if (filterStatus === "pagados") {
      // tratamos "pagado" como principal,
      // y opcionalmente "confirmado" como equivalente a pagado
